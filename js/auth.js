const Auth = (() => {
  ...
  const register = async ({ name, username, email, password }) => {
    ...
    if (validate.username(username)) {
      errors.username = validate.username(username);
    } else {
  ...
  const login = async ({ email, password }) => {
    ...
    try {
      // Usa Storage.apiAuth para que funcione tanto en local como en producción
      const result = await Storage.apiAuth({ action: 'login', email, password });
      if (result.success) {
